...
- **`forward` тільки для docker-бриджів, не для довільних інтерфейсів.**
  Список інтерфейсів для `forward` цілком походить від `get_docker_bridges()`
  (`docker network ls`/`inspect`) — фізичні NIC (напр. `eno0`↔`eno1`, хост
  як звичайний роутер між двома мережами без Docker) туди попасти не можуть,
  і сам ланцюг `forward` узагалі не створюється, якщо Docker вимкнено
  (`--no-docker`/автовизначення). При цьому сама генерація правил
  per-interface (`gen_static_guard_rules`/`gen_srv_rules_all_ports`/
  `gen_level_rules` з матчем `iifname != $iface oifname $iface`) не
  докер-специфічна — вузьке місце саме в джерелі списку інтерфейсів. Ідея:
  окремий override-файл (за аналогією з `srv_ports`) зі списком фізичних
  інтерфейсів, які `build_ruleset` домішує до результату
  `get_docker_bridges()` перед тим самим циклом.
