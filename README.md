...
- IPv6 — опційно; якщо вимкнено системно (`net.ipv6.conf.all.disable_ipv6=1`) чи в ядрі взагалі немає підтримки, `nft-scan-detector` сам вмикає `--no-ipv6`: жодних v6-сетів/правил не створюється
...
спрацюють. Вмикається і вручну, і автоматично — якщо IPv6 вимкнено
системно (`net.ipv6.conf.all.disable_ipv6=1`) чи в ядрі взагалі немає
підтримки IPv6 (`/proc/sys/net/ipv6` відсутній):

```sh
sudo nft-scan-detector --no-ipv6
```

Незалежно від глобального стану, **для кожного docker bridge окремо**
...
глобально IPv6 живий. Один IPv4-only bridge серед інших не позначається
на сусідніх: кожен перевіряється незалежно. `input` — виняток: це не
per-interface ланцюг, тож там рішення лише глобальне (`--no-ipv6` або
нічого).
