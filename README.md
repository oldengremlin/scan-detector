...
список скрізь. А `meter`-лічильники `rate`/`over` — **не** спільні: nftables
...
- **Окрема `table inet` для `srv_*` (винесення з `scanDetector`) —
  окрема ідея, теж поки не реалізована.** На відміну від пункту вище, тут
  не йдеться про довільну кількість гілок, а про рівно один додатковий,
  фіксований table (напр. `srvProtector`/`srvProtector6`) — розділення
  ДВОХ логічно різних захистів, які зараз живуть в одній таблиці:
  ескалаційна драбина `level*`/`scan` (довгий, до 31 дня, стан) і
  meter-based `srv_*` (rate/over для захищених портів, стан якого й так
  щоразу скидається на regen — див. "Відомі нюанси нової моделі" в
  [TESTS.md](TESTS.md)). Окрема таблиця дозволила б `--init`/regen для
  одного захисту, не чіпаючи стан іншого. Пріоритет нової таблиці мав би
  бути числово нижчим за `scanDetector` (напр. `-51` проти `-50`), щоб
  `srv_*` і далі спрацьовував ДО того, як пакет дійде до драбини
  `level*`/`scan` — так само, як зараз у межах однієї таблиці за порядком
  правил. `permanentBlock`/`trusted`/`ignore` довелось би продублювати в
  новій таблиці (з тих самих override-файлів) — той самий патерн, що вже
  є між `input` і кожним forward-бриджем сьогодні. Нетривіальна, але
  обмежена за обсягом зміна — поки що влаштовує як є, реалізація не
  запланована.
