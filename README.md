...
За замовчуванням — `normal:30s`, `check0:10s`, `check1:1m30s`, `block:1m`.
Формат: `stage:timeout` по рядку, рівно ці чотири стадії, кожна один раз
(порядок рядків не важливий). Неповний набір чи зайва/невідома стадія —
фолбек на дефолт цілком (не частково).

`srv_normal` — **ковзне** вікно: освіжається (`update`) на кожному новому
з'єднанні до захищеного порту, доки джерело не в `srv_block`, а не лише на
першому заході. `srv_block` наповнюється, лише якщо джерело вже в
`srv_check1`, а `srv_normal` встиг протухнути — тобто "небезпечна зона"
(пауза після сплеску, яка веде в блок) дорівнює `check1 − normal` і не
залежить від `check0` (той лише вирішує, наскільки тісним має бути сплеск
двох з'єднань, щоб узагалі потрапити в `check1`). Детальна методологія
підбору цих чисел, знайдений баг (самосуперечливе правило, через яке
`srv_block` раніше був мертвим кодом) і живі виміри — у [TESTS.md](TESTS.md).
...
- **Кілька незалежних `srv_*`-секцій у межах однієї таблиці.** Зараз усі
  порти з `/etc/nft-scandetect/srv_ports` (`tcp/80`, `tcp/443` за
  замовчуванням) пишуть в один спільний каскад `srv_normal → check0 →
  check1 → block` — throttle по `saddr` спільний для всіх портів разом,
  не per-група. Ідея: дозволити кілька незалежних груп зі своїм набором
  портів і своїм каскадом (свої `srv_<group>_normal/check0/check1/block`)
  — наприклад, окрема секція для `{ 80, 443 }` і окрема для `{ 20, 21 }`
  — які йдуть одна за одною в `input`/`forward` до основної логіки
  `scan`/`level*`, не заважаючи одна одній.
